        ...
        for (int i = 1; i <= previousRound.size(); i++) {
            if (previousRound.get(i - 1).getEliminationRound() == null ||
                ...
                if (previousRound.get(i - 1).getPosition() % 2 == 0) {
                    if (previousRound.get(i - 2).getEliminationRound() != null &&
                        previousRound.get(i - 2).getEliminationRound() <= roundNumber) {
                        nextRound.add(previousRound.get(i - 1));
                    }
                } else {
                    if (i < previousRound.size()) {
                        if (previousRound.get(i).getEliminationRound() != null &&
                            previousRound.get(i).getEliminationRound() <= roundNumber) {
                            nextRound.add(previousRound.get(i - 1));
                        }
                    }
                }
